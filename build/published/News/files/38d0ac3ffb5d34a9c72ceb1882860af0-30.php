...
<link rel="icon" type="image/png" href="https://www.adaforge.org/resources/favicon_small.png" sizes="16x16" />
	...
	<meta name="twitter:url" content="https:/www.adaforge.org/News/files/38d0ac3ffb5d34a9c72ceb1882860af0-30.php">
	...
	<meta property="og:url" content="https:/www.adaforge.org/News/files/38d0ac3ffb5d34a9c72ceb1882860af0-30.php"> <title>SymExpr 1.1 released | Programmer's Forge for Open Source Ada Code and Tools - News</title>
...
<body>
	...
	<div class="blog-archive-entries-wrapper">
		<div id="unique-entry-id-30" class="blog-entry"><h1 class="blog-entry-title">SymExpr 1.1 released</h1><div class="blog-entry-date">2016-05-22 Filed in: <span class="blog-entry-category"><a href="category-tech-library.php">Tech Library</a></span></div><div class="blog-entry-summary">SymExpr is a generic Ada package that allow you some manipulation of "symbolic expressions."<br /><br /><a href="https://launchpad.net/symexpr/+download">https://launchpad.net/symexpr/+download</a><br /><br />From <a href="mailto:framefritti@gmail.com">Riccardo Bernardini</a></div><div class="blog-entry-body">The expressions handled by SymExpr have the usual form with the four operations (+, -, * and /), the two unary operators (+ and -),<br />function calls, variables and parenthesis. The package is a generic package parameterized by the type of the "values" used in the expressions (called "scalars" in the code), so you can have expressions of floats, of integers, but also of dates (my case), complexes, and so on...<br /><br />  *  You can create new expressions by parsing strings<br /><br /><code>         X := Parse("foo+(bar-3)*max(bar, foo)")</code><br />  *  You can build expressions using constructors and operators, for <br />     example  <br /><br /><code>         X := Variable("x");         Y := X*X - 2*X + 1;</code><br />  *  You can replace variables with scalars or other expressions, for<br />     example, <br /><br /><code>         -- Replace every occurence of "x" in Y with 42         U := Replace(Y, "x", 42); </code><br /><code>         -- Makes V equal to (X*X)*(X*X) - 2*(X*X) + 1         V := Replace(Y, "x", X*X);</code><br />  *  You can evaluate an expression without variables to a scalar.<br />     For example, to plot a graph of Y you could use<br /><br /><code>         for I in -10..10 loop           Plot(I, Eval(Replace(Y, "x", I)));         end loop;</code><p class="blog-entry-tags">Tags: <a href="tag-regexpr.php" title="RegExpr" rel="tag">RegExpr</a></p></div></div>
	</div>
