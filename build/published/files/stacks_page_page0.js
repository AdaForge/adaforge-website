...
stacks.stacks_in_845 = {};
stacks.stacks_in_845 = (function(stack) {
...
$(document).ready(function() {
	if(false) {
		...
		$('<div id="atm_image_mapper_overlay_headerstacks_in_845"><div class="atm_image_mapper_areastacks_in_845"><a href="#">Finished measuring area &#39;1&#39;</a></div><div class="atm_image_mapper_showhidestacks_in_845"><a href="#">Show / Hide Placeholder</a></div><input type="checkbox" name="autohidestacks_in_845" value="autohide" /><span class="atm_image_mapper_autohidestacks_in_845"> Hide while measuring</span></div><div id="atm_image_mapper_overlaystacks_in_845"></div><div id="atm_image_mapper_coordinatesstacks_in_845"><div id="atm_image_mapper_coordinates_placeholder"><span class="atm_image_mapper_comment_text">Preview this page in desktop mode, with the stack at its maximum width. Hover your mouse over the image above to view the X &amp; Y coordinates. <br><br>To plot a map area (like a polygon) click on the image to mark each corner of the shape. When you join-up with the start point again, click <strong>finish measuring area</strong>. At this point, your measured coordinates get shown below. You can use this tool to create the coordinates for one or more of your required map areas. <br><br>Copy the coordinates to a plain text file when done. Or paste them directly into the <strong>coordinates</strong> box, of a selected map area sub-stack in edit mode.</span><br /><br /><span class="atm_image_mapper_area_textstacks_in_845">Measured coordinates area &#39;1&#39;: </span></div></div>').prependTo('body');
		$('#stacks_in_845').appendTo('#atm_image_mapper_overlaystacks_in_845').css('cursor', 'crosshair');				
		$('.atm_image_mapper_areastacks_in_845 a').data('area', parseInt(1));
		
		// Inspector Hover code.
		$('#stacks_in_845 img').hover(function(e) {
			$('<div id="atm_image_mapper_inspectorstacks_in_845"></div>').appendTo('body').css('left', (e.pageY + 20) + 'px').css('top', (e.pageY - 10) + 'px').show();
		}, function() {
			// Inspector Hover out code.
			$('#atm_image_mapper_inspectorstacks_in_845').remove();
		}).mousemove(function(e) {

			// Inspector Mouse move code.
			var atm_intX = e.pageX - Math.round($('#stacks_in_845 img').offset().left);
			var atm_intY = e.pageY - Math.round($('#stacks_in_845 img').offset().top);
			$('#atm_image_mapper_inspectorstacks_in_845').css('left', (e.pageX + 30) + 'px').css('top', (e.pageY - 20) + 'px').text('X,Y = ' + atm_intX + ',' + atm_intY).data('coordinates', atm_intX + ',' + atm_intY);
		});
		
		// Add to the Coordinates placeholder.
		$('#stacks_in_845 img').click(function(e) {
			if($('#atm_image_mapper_coordinatesstacks_in_845').length > 0) {
				if($('input[name=autohidestacks_in_845]').attr('checked')) { $('#atm_image_mapper_coordinatesstacks_in_845').hide(); }
				$('#atm_image_mapper_coordinatesstacks_in_845 #atm_image_mapper_coordinates_placeholder').append($('#atm_image_mapper_inspectorstacks_in_845').data('coordinates') + ', ');
			}
			$('<div class="atm_image_mapper_coordinatestacks_in_845"></div>').appendTo('#stacks_in_845').css('left', (e.pageX -2) + 'px').css('top', (e.pageY -2) + 'px');
		});
		
		// Add new Measuring Area.
		$('.atm_image_mapper_areastacks_in_845 a').click(function() {
			var atm_strCoordinates = new String($('#atm_image_mapper_coordinatesstacks_in_845 #atm_image_mapper_coordinates_placeholder').html());
			if(atm_strCoordinates.charAt(atm_strCoordinates.length - 2) == ",") {		
				...
				$('#atm_image_mapper_coordinatesstacks_in_845 #atm_image_mapper_coordinates_placeholder').html(atm_strCoordinates + '<span class="atm_image_mapper_area_spacestacks_in_845">&nbsp;</span><span class="atm_image_mapper_area_textstacks_in_845">Measured coordinates area &#39;' + atm_intArea + '&#39;: </span>');
				$(this).text('Finished measuring area \'' + atm_intArea + '\'');
				$('.atm_image_mapper_coordinatestacks_in_845').remove();
				if($('input[name=autohidestacks_in_845]').attr('checked')) { $('#atm_image_mapper_coordinatesstacks_in_845').show(); }
			}
			return false;
		});
		
		// Show/Hide the Coordinate placeholder.
		$('.atm_image_mapper_showhidestacks_in_845 a').click(function() {
			$('#atm_image_mapper_coordinatesstacks_in_845').toggle();
			return false;
		});	
		
		// Toggle Auto Hide.
		$('.atm_image_mapper_autohidestacks_in_845').click(function() {
			$('input[name=autohidestacks_in_845]').attr('checked', !$('input[name=autohidestacks_in_845]').attr('checked'));
		});
		
	} else {
	
		// Add the map to the image.
		$('#stacks_in_845 img').attr('usemap', '#imagemap_stacks_in_845');
		
		$('#stacks_in_845 area').attr('tabindex', function(i) {
			return (i+1);
		});
		
		// Responsive Map Feature.
		$('#stacks_in_845 img').rwdImageMaps();
...
return stack;})(stacks.stacks_in_845);